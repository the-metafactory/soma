...
- **Uniform provenance headers + content-compare doctor across all 5 substrates
  (#370)** — every generated-narrative home-projection file on codex, cursor,
  grok, and pi.dev now carries the same byte-stable `Generated by Soma` header
  claude-code already had; JSON/TOML/YAML-frontmatter/verbatim-script files and
...
- **Provenance headers + doctor drift check (#370)** — every generated
  `rules/soma` skeleton file carries a byte-stable `Generated by Soma` header
  (no timestamp, so reprojection stays byte-identical). `soma doctor
  --substrate claude-code` gains `claude-code-projection-unmanaged-edit`: a
  present, non-stale projection file that lost its header was hand-replaced and
