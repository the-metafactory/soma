...
Adaptive question-and-answer that fills in or deepens an VSA's prose sections. Used when the prompt alone doesn't carry enough signal to scaffold a strong VSA, or when the user wants to deliberately deepen an existing VSA.
...
- User directly: `Skill("VSA", "interview me on <vsa-path>")`
- User directly with a specific section: `Skill("VSA", "interview me on the Vision section of <vsa-path>")`

## Inputs

| Input | Required | Description |
|-------|----------|-------------|
| isa_path | yes | Path to the VSA to deepen |
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the Interview workflow in the VSA skill"}' \
  > /dev/null 2>&1 &
```

### Step 2 — Read the VSA

Load the VSA at `isa_path` and identify which sections are populated, thin, or missing.
...
Each question is a single Output → wait for user answer → write the answer back into the VSA section (preserving prior content; appending or replacing as appropriate).

Use plain text, no fancy formatting — this is conversation, not a form. Lead with the most foundational question first.

### Step 5 — Write back to VSA after each answer

After every user response, immediately Edit the VSA file. The user should feel the document filling up as they answer.
...
- **User answers contradict the existing VSA:** treat new answers as the canonical source. Log the contradiction in Decisions with a `refined:` prefix.
