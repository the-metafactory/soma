...
import { promotionNoteId, _promoteAlgorithmRunMemoryWithCallback } from "../src/memory-promotion";
...
// passed criterion and a verification entry. Override fields via `overrides`.
  ...
  await writeAlgorithmRun(
    {
      ...
      vsa: {
        slug: id,
        frontmatter: { task: overrides.goal ?? "Promotion", effort: "E1", mode: "algorithm", phase: "complete", progress: "1/1", verified: true, updated: "2026-05-14T12:05:00.000Z" },
        sections: [
...
test("promotes an Algorithm run into durable Soma memory", async () => {
  await withTempHome(async (homeDir) => {
    ...
    // at the PROMOTED/ file as source of truth.
...
test("refuses to promote without --principal-authority even when verified", async () => {
  await withTempHome(async (homeDir) => {
    await bootstrapSomaHome({ homeDir });
    await writeVerifiedRun(homeDir, "verified-no-auth", { goal: "Promotion refuses without --principal-authority.", intent: "Authority gate fails closed.", criterionText: "Authority required." });
...
test("promotion note id preserves the run id when the title is long", async () => {
  ...
  const longTitle = "Consulting lesson with a very long descriptive title that exceeds the sixty-four character id cap and would historically truncate the run id suffix away entirely";
...
test("promotion fires onDurable after the durable artifacts land, before bookkeeping", async () => {
  await withTempHome(async (homeDir) => {
    const { somaHome } = await bootstrapSomaHome({ homeDir });
    await writeVerifiedRun(homeDir, "durable-cb-run", { goal: "onDurable fires after note write, before event append.", intent: "Verify the durability callback fires.", criterionText: "Callback fired." });
...
test("promotion surfaces a post-durable failure instead of swallowing it", async () => {
  await withTempHome(async (homeDir) => {
    const { somaHome } = await bootstrapSomaHome({ homeDir });
    await writeVerifiedRun(homeDir, "post-durable-fail", { goal: "Post-durable failure re-throws.", intent: "Verify a post-durable throw surfaces.", criterionText: "Surfaced." });

    // Make the run file read-only AFTER setup so the run-provenance write
    // (writeAlgorithmRun, the LAST step in promoteAlgorithmRunMemory) fails.
    // The earlier steps (PROMOTED file, durable note, onDurable callback,
    // memory.promotion event) use different paths and still succeed — so this
    // isolates a POST-durable failure, the exact case the callback exists to
    // surface.
    const runFile = join(somaHome, "memory", "WORK", "algorithm-runs", "post-durable-fail.json");
    const { chmod } = await import("node:fs/promises");
    await chmod(runFile, 0o444);

    let fired = false;
    try {
      await expect(
        _promoteAlgorithmRunMemoryWithCallback({
          homeDir,
          fromRun: "post-durable-fail",
          store: "knowledge",
          title: "Post durable fail",
          principalAuthority: true,
        }, () => { fired = true; }),
      ).rejects.toThrow();
      // The callback fired BEFORE the run-provenance write failed — proving the
      // durability signal is set at the right point for a caller to re-throw.
      expect(fired).toBe(true);
    } finally {
      await chmod(runFile, 0o644);
    }
  });
});
