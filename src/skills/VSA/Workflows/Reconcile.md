...
Deterministic merge of an ephemeral feature-file excerpt back into the master VSA, keyed on stable ISC IDs. The cornerstone of the Ralph Loop / Maestro pattern — without this, ephemeral feature work drifts from master and creates the same "code drifts from spec" problem the VSA is meant to solve.
...
- The Algorithm at LEARN: `Skill("VSA", "reconcile <ephemeral-path> → <master-path>")`
- User directly: `Skill("VSA", "reconcile <ephemeral-path> → <master-path>")`
...
| master_path | yes | Path to the master VSA the ephemeral was derived from |
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the Reconcile workflow in the VSA skill"}' \
  > /dev/null 2>&1 &
```
...
- **Master VSA modified during the merge:** Reconcile is single-threaded by design. If concurrent edits happen, the second Reconcile sees a different state and may report "ISC already checked" — that's correct behavior.
