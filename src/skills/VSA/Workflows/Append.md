...
Canonical writer for the three append-only sections of an VSA: `## Decisions`, `## Changelog`, `## Verification`. The Deutsch conjecture/refutation/learning Changelog format is novel and easy to mangle with free-form editing — this workflow owns the canonical entry shape so it doesn't degrade across projects.

## When to invoke

- Algorithm at any phase when a non-obvious decision is made: `Skill("VSA", "append decision to <vsa-path>: <text>")`
- Algorithm at LEARN when understanding evolved: `Skill("VSA", "append changelog to <vsa-path>: <conjecture> / <refutation> / <learning>")`
- Algorithm at EXECUTE/VERIFY when an ISC passes: `Skill("VSA", "append verification to <vsa-path>: <ISC-N> <evidence>")`
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the Append workflow in the VSA skill"}' \
  > /dev/null 2>&1 &
```

### Step 2 — Resolve target VSA and section

Read the VSA at `isa_path`. Find the target section (`## Decisions` | `## Changelog` | `## Verification`). If the section doesn't exist, create it in the canonical position (after `## Features` for Decisions, after Decisions for Changelog, last for Verification).
...
Edit the VSA: append the entry to the end of the target section, preserving prior entries. Update frontmatter `updated: <ISO-8601>`.
