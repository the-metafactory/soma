...
5. Resolve via `references/parameter-schema.md`
...
**Doctrine note:** the whitelist exists because the twelve-section VSA frame and the `Skill("VSA")` invocation pattern raise the OBSERVE-phase floor. Without an explicit whitelist, an E1 task could "feel like" a fast-path candidate to the model and bypass the skill silently — the same under-cut already closed once at the mode-selection layer. A heuristic-shaped bypass becomes a doctrine-evasion route; an enumerated one cannot. This is that enforcement pattern, applied one level deeper.
