...
class GitLabGraphStore implements GraphStore<GitLabCreateData> {
  ...
  async createNode(spec: CreateNodeSpec<GitLabCreateData>, rehome?: RehomeSelection): Promise<NodeRef> {
    ...
    const hydrated = rehome !== undefined && typeof context === "object" && context !== null && (context as Partial<GitLabRehomeContext>)[GITLAB_REHOME] === true && rehome.parent.id === spec.parent?.id && rehome.relatedTo.id === spec.relatedTo?.id ? context as GitLabRehomeContext : undefined;
    const parent = spec.parent === undefined ? undefined : hydrated?.parent ?? await this.item(spec.parent);
    const related = spec.relatedTo === undefined ? undefined : hydrated?.related ?? await this.item(spec.relatedTo);
  ...
  async readComment(ref: CommentRef): Promise<CommentRef> { const p = parts({ id: ref.nodeId }); if (p.sigil === "&") { const note = await this.epicNote(ref); return { id: str(note, "id", "GitLab note"), nodeId: ref.nodeId, author: username(note.author), ...(typeof note.url === "string" ? { url: note.url } : {}) }; } const note = rec(await this.transport({ method: "GET", path: `${restIssue(p)}/notes/${ref.id}` }), "GitLab note"); return { id: String(num(note, "id", "GitLab note")), nodeId: ref.nodeId, author: username(note.author), ...(typeof note.web_url === "string" ? { url: note.web_url } : {}) }; }
