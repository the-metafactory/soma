...
function renderHomeExtension(somaHome: string): string {
  return [
    ...
    "}",
    "",
    "function runSomaMemorySearch(query: string, limit?: number): string {",
    ...
    "}",
    "",
    "function runSomaMemoryPromote(params: { fromRun?: string; store?: string; title?: string; lesson?: string; appliesWhen?: string }): string {",
    '\tif (!params.fromRun || !params.store || !params.title) return "Error: fromRun, store, and title are required for memory_promote";',
    '\tconst args = ["run", "soma", "memory", "promote", "--soma-home", SOMA_HOME, "--from-run", params.fromRun, "--store", params.store, "--title", params.title];',
    ...
    "\t\t\t\treturn { content: [{ type: \"text\", text: runSomaMemorySearch(params.query, params.limit) }] };",
    "\t\t\t}",
    "\t\t\tif (params.action === \"memory_promote\") {",
    "\t\t\t\treturn { content: [{ type: \"text\", text: runSomaMemoryPromote(params) }] };",
    "\t\t\t}",
