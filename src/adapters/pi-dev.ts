...
function renderHomeExtension(somaHome: string): string {
  return [
    ...
    '\t\tconst startupContext = readOptional(`${PI_SOMA_HOME}/startup-context.md`);',
    ...
    '\t\t\t\t\t: params.action === "startup_context"',
    '\t\t\t\t\t\t? `${PI_SOMA_HOME}/startup-context.md`',
    '\t\t\t\t\t\t: params.action === "algorithm_work_index"',
    '\t\t\t\t\t\t\t? `${SOMA_HOME}/memory/STATE/algorithm-work-index.json`',
