...
Generate a fresh VSA from a prompt at a specified effort tier. The output is a populated VSA file at the canonical location with all required sections per tier.

## When to invoke

- The Algorithm at OBSERVE: `Skill("VSA", "scaffold from prompt: <user message> at tier <tier>")`
- User directly: `Skill("VSA", "scaffold from prompt: <prompt>")` — defaults tier to E3 if unspecified
- Ephemeral feature mode: `Skill("VSA", "extract feature <name> as ephemeral file from <master-vsa-path>")`
...
| project | no | If task targets a known project from PROJECTS.md, the project VSA path is used; otherwise a task VSA at `<soma-home>/memory/WORK/{slug}/VSA.md` |
| ephemeral_feature | no | If set, scaffold a feature-file excerpt instead of a full VSA |

## Output

A markdown file at one of:
- `<project-root>/VSA.md` — when `project` is supplied (existing project VSA is read-extended, not overwritten)
- `<soma-home>/memory/WORK/{slug}/VSA.md` — when no project (slug = `YYYYMMDD-HHMMSS_kebab-task-description`)
- `<soma-home>/memory/WORK/{slug}/_ephemeral/<feature>.md` — when `ephemeral_feature` is set

`<soma-home>` is the configured shared Soma home. Claude Code must write and sync task VSAs there instead of creating a parallel Claude-local work copy.
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the Scaffold workflow in the VSA skill"}' \
  > /dev/null 2>&1 &
```
...
**Project VSA override:** if `<project>/VSA.md` is the target, require E3+ sections regardless of the active task's tier.
...
Before returning, invoke `Workflows/CheckCompleteness.md` against the new VSA at the requested tier. If any required section is missing, fill it before declaring the scaffold complete.

### Step 10 — Return the path

Output the absolute path of the created VSA file. Algorithm OBSERVE consumes this path.

## Ephemeral feature mode

When `ephemeral_feature` is set:

1. Read the master VSA at `master_isa_path`.
...
5. Add a header comment: `<!-- EPHEMERAL FEATURE FILE — derived from <master-vsa-path>. Reconcile via Skill("VSA", "reconcile <this-path> → <master-path>"). Do not hand-edit master from this file. -->`
...
- **ID collision in ephemeral mode:** if the feature's ISC IDs don't exist in master, abort and surface the inconsistency — this is a master-VSA error, not a Scaffold error.
