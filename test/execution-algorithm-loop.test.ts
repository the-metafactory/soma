...
function event(kind: "execution.completed" | "execution.failed" | "execution.cancelled") {
  return kind === "execution.completed"
    ...
    : [{ kind: "execution.started" as const, executionId: "$executionId", timestamp }, kind === "execution.failed" ? { kind, executionId: "$executionId", timestamp, code: "substrate-exit" as const, summary: "failed", retryable: false } : { kind, executionId: "$executionId", timestamp, summary: "cancelled" }];
}
...
test("validated executions write bounded lifecycle evidence without advancing criteria", async () => {
  ...
  try {
    ...
    const bridge = new SubstrateExecutionAlgorithmLoopExecutor({ executor: mock, somaHome, kernelOptions: { authorizedWorkspaceRoot: "/tmp" }, timestamp: () => timestamp, request: (context) => ({ taskId: context.iteration === 1 ? "complete" : context.iteration === 2 ? "failed" : "cancelled", substrate: "codex", prompt: "private", cwd: "/tmp", projectionFingerprint: "projection", requiredCapabilities: [], algorithmRunId: context.run.id }) });
...
test("writeback happens only after the kernel returns a validated terminal result", async () => {
  ...
  try {
    ...
    const bridge = new SubstrateExecutionAlgorithmLoopExecutor({ executor: mock, somaHome, kernelOptions: { authorizedWorkspaceRoot: "/tmp" }, timestamp: () => timestamp, request: () => ({ taskId: "refused", substrate: "codex", prompt: "private", cwd: "/tmp", projectionFingerprint: "projection", requiredCapabilities: [] }) });
