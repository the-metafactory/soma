...
function renderCodexPolicyHookRuntime(somaHome: string): string[] {
  ...
  return [
    ...
    '  const args = ["run", "soma", "policy", "check-batch", "--soma-home", SOMA_HOME, "--substrate", "codex", "--action", "write", "--targets-env", "SOMA_POLICY_TARGETS", "--record", "deny", "--json"];',
    "",
...
function renderCodexLifecycleHook(somaHome: string): string {
  return [
    ...
    "  const targets = extractWriteTargets(input).filter((target) => hasSomaPolicyMarker(target.content));",
    "  if (targets.length > 0) {",
