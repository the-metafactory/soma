...
export function renderSomaAlgorithmExtension(options: RenderSomaAlgorithmExtensionOptions = {}): string {
  ...
  const policyModule = JSON.stringify(new URL("../../../policy-audit.ts", sourceDir).href);
  const pathGuardModule = JSON.stringify(new URL("../../../policy-path-guard.ts", sourceDir).href);
  const runSnapshotModule = JSON.stringify(new URL("../algorithm-run-snapshot.ts", sourceDir).href);
