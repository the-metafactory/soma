...
- **✅ DECISION RESOLVED 2026-08-09 — reading (a): Soma replaces the PAI name.** The importer no longer emits the product name into anything that projects. `src/pai-importer.ts` dropped the `- source: Claude PAI {principal,DA} identity` TRAIT lines from the `## Profile` blocks — traits are what every substrate renders, which is why "PAI" appeared in every session's context on every substrate. Provenance did not move: the non-projecting `## Source` section still records `Migrated from ~/.claude/<path>`. Also neutralised the fallback mission (it PROJECTS, so a source with no mission line would have shown the importer's own branding as the principal's mission) and the TELOS migration note. Input paths (`PAI/USER/...`) and error strings keep the name deliberately — the importer reads a PAI install, and saying so is accurate. See PR #588.
