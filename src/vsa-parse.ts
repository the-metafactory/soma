...
import { getCriteria, progressFromCriteria, verifiedFromCriteria } from "./vsa-accessors";

/**
 * Round-trip contract: parseVsa → serializeVsa produces **semantic equivalence**
...
const RAW_INPUT = new WeakMap<VerificationStateArtifact, string>();
...
export function parseVsa(markdown: string, sourcePath?: string): VerificationStateArtifact {
  ...
  const isa: VerificationStateArtifact = { slug, frontmatter, sections, sourcePath };
  RAW_INPUT.set(isa, markdown);
  return isa;
}

/**
 * If `isa` is the exact identity returned by `parseVsa`, return the raw input
 * verbatim — no structural mutation occurred (the helpers in `vsa-accessors`
 ...
 */
export function serializeVsa(isa: VerificationStateArtifact): string {
  const previous = RAW_INPUT.get(isa);
  if (previous !== undefined) {
    return previous;
  }
  const frontmatter = withRecomputedDerived(isa);
  return renderVsa({ ...isa, frontmatter });
}

function renderVsa(isa: VerificationStateArtifact): string {
...
function renderSection(section: VsaSection): string {
  const content = section.content.replace(/\s+$/, "");
  return `## ${section.name}\n\n${content}`;
}

function withRecomputedDerived(isa: VerificationStateArtifact): VsaFrontmatter {
...
function parseFrontmatter(raw: string): VsaFrontmatter {
...
function renderFrontmatter(frontmatter: VsaFrontmatter): string {
  const lines: string[] = ["---"];
...
function parseSections(body: string): readonly VsaSection[] {
