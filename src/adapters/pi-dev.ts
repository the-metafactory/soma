...
function renderHomeExtension(somaHome: string): string {
  return [
    ...
    "\tif (!prompt) return \"# Soma Prompt Classification\\nPrompt unavailable from Pi.dev event; use the-algorithm manually for substantial work.\";",
    "\tconst result = runSomaClassification(prompt);",
    "\tif (!result.ok) return `# Soma Prompt Classification\\nClassification failed; use the-algorithm manually for substantial work. ${result.output}`;",
