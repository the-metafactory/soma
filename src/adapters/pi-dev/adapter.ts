...
function renderHomeExtension(somaHome: string): string {
  return [
    ...
    "",
    "// ── Cached state (avoids synchronous Soma calls on every message) ──",
    "// Startup context: computed once at session_start, reused for all",
    "// subsequent before_agent_start calls within the session.",
    "let cachedStartupContext: string | undefined;",
    "",
    "// Classification cache: computed in turn_end (after each response),",
    "// injected into the NEXT turn's system prompt via before_agent_start.",
    "// First message of a session has no classification (no blocking),",
    "// every subsequent message gets the previous turn's classification.",
    "let cachedClassification: string | undefined;",
    ...
    '\taction: StringEnum(["profile", "startup_context", "algorithm_work_index", "memory_layout", "pai_imports", "source_file", "memory_search", "memory_recall", "memory_index"] as const),',
    ...
    "\t}",
    "",
    "async function runSomaClassificationAsync(prompt: string): Promise<{ ok: boolean; output: string }> {",
    "\ttry {",
    '\t\tconst result = await execFileAsync("bun", ["run", "soma", "algorithm", "classify", "--prompt", prompt], {',
    "\t\t\tcwd: somaRepoPath(),",
    '\t\t\tencoding: "utf8",',
    "\t\t\ttimeout: 25000",
    "\t\t});",
    ...
    "}",
    "",
    "// Render classification context from already-parsed classification data.",
    "function renderPromptClassificationContext(prompt: string, classification: Record<string, string>): string {",
    ...
    "}",
    "",
    "// Refresh startup context (async — called at session_start only).",
    ...
    "}",
    "",
    "// Refresh work index (async — called at agent_end and tool_execution_end).",
    "async function refreshWorkIndexAsync(): Promise<void> {",
    "\tawait runSomaLifecycleAsync(\"algorithm-updated\").catch(() => {});",
    "}",
    "",
    "function captureSessionEnd(sessionId?: string): void {",
    "\t// Session-end: best-effort, fire-and-forget. Don't block shutdown.",
    "\trunSomaLifecycleAsync(\"session-end\", sessionId).catch(() => {});",
    "}",
    "",
    ...
    "\t\t// Compute startup context ONCE per session, cache for before_agent_start.",
    "\t\tcachedStartupContext = await refreshStartupContext(sessionId(ctx));",
    "\t\t// Defer work index refresh so it doesn't block session startup.",
    "\t\tsetImmediate(() => { refreshWorkIndexAsync().catch(() => {}); });",
    ...
    "\t\t// Use cached startup context (computed once at session_start).",
    "\t\tconst startupContext = cachedStartupContext ?? \"\";",
    '\t\tconst paiImports = readOptional(`${PI_SOMA_HOME}/pai-imports.md`);',
    '\t\tconst context = readOptional(`${PI_SOMA_HOME}/context.md`);',
    "\t\t// Use cached classification from previous turn (if any). First message of session: no classification (no blocking). Subsequent messages: classification precomputed in turn_end.",
    "\t\tconst promptClassification = cachedClassification ?? \"\";",
    '\t\tconst somaPrompt = `',
    ...
    "",
    "",
    "\\t// turn_end: classify the prompt that was just processed. Result cached for the NEXT turn before_agent_start.",
    "\\t// First message of session has no classification (fast response). Every subsequent message",
    "\\t// gets classification context without any blocking (computed after previous response).",
    '\tpi.on("turn_end", async (event) => {',
    "\\t\\tconst prompt = promptFromEvent(event);",
    "\\t\\tif (!prompt) return;",
    "\\t\\t// Classify the prompt. Result cached for next turn before_agent_start.",
    "\\t\\trunSomaClassificationAsync(prompt).then(({ ok, output }) => {",
    "\\t\\t\\tif (!ok) {",
    '\t\t\t\tcachedClassification = "Soma: classification failed. Use the-algorithm manually.";',
    "\\t\\t\\t\\treturn;",
    "\\t\\t\\t}",
    "\\t\\t\\tconst classification = parseClassification(output);",
    "\\t\\t\\tcachedClassification = renderPromptClassificationContext(prompt, classification);",
    "\\t\\t}).catch(() => {",
    ...
    '\t\tif (/(edit|write|patch|bash|shell)/.test(name)) {',
    "\\t\\t\\tsetImmediate(() => { refreshWorkIndexAsync().catch(() => {}); });",
    "\\t\\t}",
    "\t});",
    "",
