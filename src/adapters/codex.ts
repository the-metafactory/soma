...
function renderHomeSkill(input: SomaContextInput, somaHome: string): string {
  return [
    ...
    "- Use `cd $(cat ~/.codex/memories/soma/soma-repo.txt) && bun run soma memory promote --from-run <id> --store learning --title \"...\"` when verified Algorithm work should become durable recall.",
...
function renderLifecycleProjection(somaHome: string): string {
  return [
    ...
    "- `cd $(cat ~/.codex/memories/soma/soma-repo.txt) && bun run soma memory promote --from-run <id> --store learning --title \"...\"`",
    "",
