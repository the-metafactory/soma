...
function renderHomeExtension(somaHome: string): string {
  return [
    ...
    '\taction: StringEnum(["profile", "startup_context", "algorithm_work_index", "memory_layout", "pai_imports", "source_file", "memory_search", "memory_promote"] as const),',
    ...
    "\t\t\t}",
    "\t\t\tif (params.action === \"memory_promote\") {",
...
function renderHomeSkill(input: SomaContextInput, somaHome: string): string {
  return [
    ...
    "- Use the `soma_context` tool with `action: memory_promote` when verified Algorithm work should become durable recall.",
