...
test("AC-4 property: reconcile(master, master) is identity", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { Notes: "Keep me" });
  const result = reconcileVsaArtifacts(master, master);
  expect(serializeVsa(result.isa)).toBe(serializeVsa(master));
  expect(result.report.changed).toBe(false);
});

test("AC-4 property: no-op reconcile ignores timestamp-only changes", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { Notes: "Keep me" });
  const result = reconcileVsaArtifacts(master, master, { timestamp: "2026-05-17T11:00:00.000Z" });
  expect(serializeVsa(result.isa)).toBe(serializeVsa(master));
  expect(result.report.changed).toBe(false);
});
...
test("AC-4 property: no-op reconcile preserves existing log sections", () => {
  ...
  const master = buildVsa("demo", [criterion("ISC-1", "open")], {
...
test("AC-5 property: reconcile is idempotent for the same feature", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")]);
  const feature = buildVsa("demo", [criterion("ISC-1", "passed", "ISC-1 works", "bun test passed"), criterion("ISC-2", "open")], {
    Notes: "Feature notes",
  });
  const once = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-feature" }).isa;
  const twice = reconcileVsaArtifacts(once, feature, { onConflict: "prefer-feature" }).isa;
  expect(serializeVsa(twice)).toBe(serializeVsa(once));
});

test("AC-6 correctness: stable ISC IDs from feature land in master criteria", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")]);
  const feature = buildVsa("demo", [criterion("ISC-1", "passed", "ISC-1 works", "evidence"), criterion("ISC-2", "open")]);
  const result = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-feature" });
...
test("invalid conflict policy rejects before merge resolution", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "passed", "Done", "evidence")]);
  const feature = buildVsa("demo", [criterion("ISC-1", "open", "Done")]);
  expect(() => reconcileVsaArtifacts(master, feature, { onConflict: "prefer-mistake" as never })).toThrow("Invalid VSA reconcile conflict policy");
});

test("adversarial 1: section added in feature appends when absent from master", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { [SECTION_NAME_MAP.constraints]: "Master constraint" });
  const feature = buildVsa("demo", [criterion("ISC-1", "open")], { Research: "Feature evidence" });
  const result = reconcileVsaArtifacts(master, feature);
...
test("adversarial 2: duplicate ISC IDs in master reject file-backed reconcile", async () => {
  await withSomaHome(async (homeDir) => {
    await scaffoldVsa({ homeDir, slug: "dup", goal: "G", effort: "E1", initialCriteria: [criterion("ISC-1", "open")] });
    const master = buildVsa("dup", [criterion("ISC-1", "open")], { Extra: "- [ ] ISC-1: Duplicate" });
    await writeVsa("dup", master, { homeDir });
    const feature = buildVsa("dup", [criterion("ISC-2", "open")]);
    await expect(reconcileVsa("dup", feature, { homeDir })).rejects.toThrow("criterion-duplicate");
  });
});

test("adversarial 3: manual master edits are preserved under prefer-master", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { [SECTION_NAME_MAP.goal]: "Manual master goal" });
  const feature = buildVsa("demo", [criterion("ISC-2", "open")], { [SECTION_NAME_MAP.goal]: "Old feature goal" });
  const result = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-master" });
...
test("adversarial 7: conflicting Decisions timestamp errors unless policy resolves", () => {
  ...
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { [SECTION_NAME_MAP.decisions]: decisions(entryA) });
  const feature = buildVsa("demo", [criterion("ISC-1", "open")], { [SECTION_NAME_MAP.decisions]: decisions(entryB) });
  const errored = reconcileVsaArtifacts(master, feature);
  expect(errored.report.conflicts.some((c) => c.resolution === "error" && c.kind === "log-entry")).toBe(true);
  const resolved = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-feature" });
...
test("adversarial 7: duplicate feature log keys merge at most one entry", () => {
  ...
  const feature = buildVsa("demo", [criterion("ISC-1", "open")], { [SECTION_NAME_MAP.decisions]: decisions(entryA, entryB) });
  const resolved = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-feature" });
...
test("adversarial 8: tombstoned criteria are preserved and never resurrected", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "dropped", "[DROPPED - see Decisions]")]);
  const feature = buildVsa("demo", [criterion("ISC-1", "passed", "Resurrected", "evidence")]);
  const result = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-feature" });
...
test("feature-side dropped status obeys conflict policy", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "passed", "Done", "evidence")]);
  const feature = buildVsa("demo", [criterion("ISC-1", "dropped", "Drop it")]);
  const preferMaster = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-master" });
  ...
  const preferFeature = reconcileVsaArtifacts(master, feature, { onConflict: "prefer-feature" });
...
test("adversarial 9: likely section rename conflicts instead of duplicating", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { "Acceptance Criteria": "Same body" });
  const feature = buildVsa("demo", [criterion("ISC-1", "open")], { "Verification Criteria": "Same body" });
  const result = reconcileVsaArtifacts(master, feature);
...
test("error-policy conflicts return the original master artifact", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "passed", "Done", "evidence")], { Keep: "Master" });
  const feature = buildVsa("demo", [criterion("ISC-1", "open", "Done"), criterion("ISC-2", "open")], { Added: "Feature" });
  const result = reconcileVsaArtifacts(master, feature);
...
test("correctness: master content untouched by feature stays present", () => {
  const master = buildVsa("demo", [criterion("ISC-1", "open")], { [SECTION_NAME_MAP.constraints]: "Do not lose this" });
  const feature = buildVsa("demo", [criterion("ISC-2", "open")]);
  const result = reconcileVsaArtifacts(master, feature);
