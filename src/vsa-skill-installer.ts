...
// home-projection adapters can recognize the VSA skill and delegate its file
// projection to this dedicated installer instead of double-emitting it through
// the generic portable-skill loop.
export const VSA_SKILL_NAME = "VSA";
const SKILL_NAME = VSA_SKILL_NAME;
 ...
 * unqualified "VSA" key for backwards-compat with pre-#37 baselines.
 */
...
function resolveSomaHome(options: Pick<VsaSkillInstallOptions, "homeDir" | "somaHome"> = {}): string {
  return resolve(options.somaHome ?? join(options.homeDir ?? homedir(), ".soma"));
}

function resolveSomaRepoPath(options: Pick<VsaSkillInstallOptions, "somaRepoPath"> = {}): string {
  return resolve(options.somaRepoPath ?? defaultSomaRepoPath());
}

export function vsaSkillSourceDir(somaRepoPath: string): string {
  return join(somaRepoPath, SOURCE_SUBPATH);
}

export function vsaSkillRuntimeDir(somaHome: string): string {
...
async function writeKnownSkillFile(
  ...
  if (content === undefined) {
    throw new Error(`vsa-skill-installer: no content for skill file ${rel}`);
  }
...
/**
 * The single source of truth for VSA skill file CONTENT. Lists the source
 * files (sorted) and applies the substrate transform to each, returning
 * {rel, content} entries. Every VSA projection derives its bytes from here —
 * the on-disk install writes (freshInstall / reconcileSameVersion), the
 * baseline hashes, and the in-memory export bundle (projectVsaSkillBundleFiles)
...
/**
 * Install or upgrade the VSA skill into the Soma home.
 *
 * States:
 ...
 *     marker, no overwrite. User decides via `soma vsa skill upgrade` (#36).
 *   - `force: true` → behave as fresh install regardless of state.
 */
export function installVsaSkill(options: VsaSkillInstallOptions = {}): Promise<VsaSkillInstallResult> {
  return installVsaSkillInternal(options);
}

export function installVsaSkillProjection(options: InternalVsaSkillInstallOptions = {}): Promise<VsaSkillInstallResult> {
  return installVsaSkillInternal(options);
}
...
/**
 * Pure in-memory projection of the VSA skill for a substrate bundle.
 *
 * Returns the same file CONTENT `installVsaSkillProjection` writes under
 ...
 * file set matches an installed home: the VSA skill has a
 ...
 * VSA skill while its files are absent.
 *
...
async function installVsaSkillInternal(options: InternalVsaSkillInstallOptions = {}): Promise<VsaSkillInstallResult> {
  ...
  const skillKey = baselineKey(runtimeDir, vsaSkillRuntimeDir(somaHome));
...
async function reconcileSameVersion(ctx: ReconcileSameVersionContext): Promise<VsaSkillInstallResult> {
...
async function writeUpgradeMarker(ctx: UpgradeMarkerContext): Promise<VsaSkillInstallResult> {
...
async function freshInstall(ctx: FreshInstallContext): Promise<VsaSkillInstallResult> {
  const written: string[] = [];
