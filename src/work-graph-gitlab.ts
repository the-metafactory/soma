...
class GitLabGraphStore implements GraphStore {
  ...
  private async items(refs: readonly NodeRef[]): Promise<Item[]> { if (refs.length === 0) return []; const parsed = refs.map(parts); if (refs.length === 1) { const p = parsed[0]!; const response = await this.transport({ method: "POST", path: "graphql", body: { query: ITEM_QUERY, variables: { fullPath: p.path, iid: String(p.iid) } } }); const namespace = rec(gqlValue(response, "namespace"), "work item namespace"); const item = itemFrom(namespace.workItem, `work item ${refs[0]!.id}`, p.path); if (item.linksTruncated) throw new WorkGraphError("backend", `GitLab blockers for ${refs[0]!.id} are paginated; refusing incomplete graph state`); return [item]; }
    const variables: Record<string, string> = {}; const definitions: string[] = []; const selections: string[] = [];
    for (const [index, p] of parsed.entries()) { variables[`path${index}`] = p.path; variables[`iid${index}`] = String(p.iid); definitions.push(`$path${index}:ID!,$iid${index}:String!`); selections.push(`item${index}:namespace(fullPath:$path${index}){workItem(iid:$iid${index}){${ITEM_FIELDS}}}`); }
  ...
  async createNode(spec: CreateNodeSpec): Promise<NodeRef> {
    ...
    if (parent === undefined) {
      ...
      input = { namespacePath: group, workItemTypeId: "gid://gitlab/WorkItems::Type/8", title: spec.title, descriptionWidget: { description } };
    } else {
      ...
      input = { projectPath: homeProject, workItemTypeId: type === "Issue" ? "gid://gitlab/WorkItems::Type/1" : "gid://gitlab/WorkItems::Type/5", title: spec.title, descriptionWidget: { description }, hierarchyWidget: { parentId: parent.id }, ...(related === undefined ? {} : { linkedItemsWidget: { linkType: "RELATES_TO", workItemsIds: [related.id] } }) };
    }
  ...
  async close(ref: NodeRef, receipt: CloseReceipt, expectedGatedNodeHash?: string): Promise<void> { const state = await this.readNode(ref); const hash = hashGatedNodeFields(state.node); if (expectedGatedNodeHash !== undefined && hash !== expectedGatedNodeHash) throw new WorkGraphError("invalid-node", `node ${ref.id} changed after close validation`); const boundReceipt = { ...receipt, autonomy: state.node.autonomy, gatedNodeHash: hash }; const posted = await this.postComment(ref, renderCloseReceipt(boundReceipt)); if (posted.author === undefined || posted.author.length === 0) throw new WorkGraphError("backend", "posted close receipt has no authenticated author"); const completion = { receiptCommentId: posted.id, checkpointId: receipt.checkpointId, autonomy: receipt.autonomy ?? state.node.autonomy, closer: posted.author, closedAt: boundReceipt.at, gatedNodeHash: hash, ...(state.node.autonomy === "auto" ? { autoProbeKeys: (state.node.probes ?? []).map((probe) => JSON.stringify(probe)).sort() } : {}), ...(receipt.ci === undefined ? {} : { ciCheckRunId: receipt.ci.checkRunId, ciHeadSha: receipt.ci.headSha }) }; const raw = await this.readRawBody(ref); const text = decodeNodeBlock(raw).text; const body = `${text}${text === "" ? "" : "\n\n"}${encodeNodeBlock({ ...state.node, title: state.node.title, completion })}`; const p = parts(ref); if (p.sigil === "&") { await this.updateEpic(ref, body, true); return; } await this.transport({ method: "PUT", path: restIssue(p), body: { description: body, state_event: "close" } }); }
}
