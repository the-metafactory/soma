...
const TOKEN_KEYS = ["GITLAB_TOKEN", "GLAB_TOKEN", "GITLAB_ACCESS_TOKEN", "OAUTH_TOKEN", "CI_JOB_TOKEN", "GITLAB_API_HOST", "GLAB_HOST", "GITLAB_HOST"] as const;
...
class GitLabGraphStore implements GraphStore {
  ...
  async createNode(spec: CreateNodeSpec): Promise<NodeRef> {
    ...
    const related = spec.relatedTo === undefined ? undefined : await this.item(spec.relatedTo);
    ...
    if (parent === undefined) {
      ...
      input = { projectPath: home, workItemTypeId: type === "Issue" ? "gid://gitlab/WorkItems::Type/1" : "gid://gitlab/WorkItems::Type/5", title: spec.title, description, hierarchyWidget: { parentId: parent.id }, ...(related === undefined ? {} : { linkedItemsWidget: { linkType: "RELATES_TO", workItemsIds: [related.id] } }) };
    }
