...
Score an existing VSA against the tier completeness gate and return a structured pass/fail + gap report. Drives the hard tier gate at all tiers.

## When to invoke

- Algorithm at end of OBSERVE: confirm the scaffolded VSA meets tier requirements.
- Algorithm at start of VERIFY: confirm the VSA is still complete after any structural changes.
- User directly: `Skill("VSA", "check completeness of <vsa-path> at tier <tier>")`
...
|-------|----------|-------------|
| isa_path | yes | Path to the VSA to score |
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the CheckCompleteness workflow in the VSA skill"}' \
  > /dev/null 2>&1 &
```
...
Project VSA (`<project>/VSA.md`) — bump tier to max(declared-tier, E3).
...
- **Project VSA scored at task tier:** override to max(tier, E3). Report the override in the output.
