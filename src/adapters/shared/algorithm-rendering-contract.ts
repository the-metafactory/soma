...
export function renderAlgorithmRenderingContract(substrateLabel: string): string {
  return [
    "---",
    "name: the-algorithm",
    'description: "Use when work should run through Soma Algorithm mode with seven-phase rendering, VSA criteria, verification, and learning capture."',
    "metadata:",
    ...
    "# The Algorithm",
    "",
    "Use this skill whenever Soma classifies the prompt as ALGORITHM or the user explicitly asks for the Algorithm, VSA, ideal state, criteria, or verification-first work.",
    "",
    "## Execution Harness",
    "",
    ...
    "",
    "OBSERVE has a current-state floor: `algorithm advance` will not leave OBSERVE until at least one current-state probe is on record. Confirm each named field, flag, route, or file actually exists — probe reality, do not trust the spec — and record it with `algorithm observe --claim <assumption> --evidence <what you found> --evidence-kind probed` (or `tested`). A `specified` observation only restates the spec and does NOT clear the floor.",
    "",
    ...
    "## Phase Rules",
    "",
    "- OBSERVE: restate task, current state, ideal state, effort, criteria. Probe every current-state assumption (named field, flag, route, file) and record at least one with `algorithm observe … --evidence-kind probed` before advancing — the gate blocks OBSERVE→THINK without it.",
    ...
    "",
    "━━━ 👁️ OBSERVE ━━━ 1/7",
    "Current state, goal, and criteria are identified; each assumption is probed (e.g. the route exists — confirmed by grep) and recorded with `algorithm observe … --evidence-kind probed`.",
    "",
    "━━━ 🧠 THINK ━━━ 2/7",
