...
- **✅ DECISION RESOLVED 2026-08-09 — reading (a): Soma replaces the PAI name.** Implemented in PR #589, but note precisely what that PR could and could not reach, because reading (a) as framed below spans both. **What the importer EMITS (its own string literals) — fixed:** the `- source: Claude PAI {principal,DA} identity` TRAIT lines are gone from the `## Profile` blocks. Traits are what an adapter renders, which is why the name reached every session on every substrate; the non-projecting `## Source` section still records `Migrated from ~/.claude/<path>`, so provenance did not move. Also neutralised the fallback mission (it PROJECTS, so a source with no mission line would have shown the importer's own branding as the principal's mission) and the TELOS migration note. A test asserts against a real `soma install claude-code --apply` projection, not just the source file. **What the importer COPIES (parsed values) — NOT code, and deliberately untouched:** `focus` is read verbatim from the source's `**Focus:**` line (`pai-importer.ts:147`), so `focus: AI infrastructure (PAI)` was the principal's own words, not an emission. Silently rewriting a principal's stated focus would be the wrong fix. It has to be corrected in the DATA — either the PAI source file or the live `~/.soma/profile/principal.md` — and no repo change can enforce that: an import from a source that still says `(PAI)` will carry it through. (Observed clean on the authoring machine 2026-08-09, but that is a machine state, not a property of this repo, and re-importing can undo it.) **Input paths (`PAI/USER/...`) and error strings keep the name deliberately:** the importer reads a PAI install, and saying so is accurate.
- *(Original framing, kept for the record.)*  "PAI" = Personal AI Infrastructure, and it's the name of JC's own active project (Projects P4 "KAI/PAI (Ivy)", Telos G1 "KAI/PAI"). So layer-3 "PAI" may be a **legitimate project name to KEEP**, not residue. Two readings to resolve with JC: (a) Soma *replaces* the PAI name → strip "(PAI)" from `focus`, clean provenance/migration notes; (b) PAI stays the *name of the infra project*, Soma is the portable kernel underneath → leave it. Provenance lines (`source: Claude PAI … identity`, `Migrated from …`) are harmless historical breadcrumbs regardless.
...
- `src/types.ts` `ProjectionInput.bundledSkillNames` + `src/adapters/shared/index.ts` `projectableSkills(skills, bundledNames?)` — filters to bundled `basename(skill.path)` when provided (install path), else legacy all-minus-VSA (direct/test callers). All 5 adapters pass `input.bundledSkillNames`; the 4 sharing the loop go through the extracted `buildPortableSkillFiles`, while **pi-dev threads it into its own id-normalizing `buildPiDevPortableSkillFiles`** — covered only indirectly (via `expectPlanCoversApplyModuloBundledSkills`), NOT by a pi-dev-specific bundled-scope test. Worth a dedicated pi-dev test.
...
**Gotcha for next session:** codex/grok double-write `skills/the-algorithm/SKILL.md` (portable copy THEN static rendering-contract render overwrites — anticipated by grok's own comment; static wins by array order). This is why codex's projected file count is 28 not 27. It is harmless ONLY while the two paths emit identical bytes: if the static rendering contract ever diverges from the portable `src/skills/the-algorithm/SKILL.md`, disk reflects the static render while the (separately computed) content differs from the portable copy — a latent desync. Mitigant today: grok EXCLUDES the-algorithm from its manifest (`GROK_PROJECTED_SKILL_NAMES` → dir-removed statically) and codex has no manifest, so the manifest-hash-desync corruption vector Sage flagged does not currently fire — but a future substrate that manifest-tracks a statically-rendered skill WOULD hit it.
