...
The VSA is the document that articulates "done" for work whose ideal state is
...
1. Identify the requested VSA workflow from the routing table below.
...
Use when the prompt mentions ideal state, VSA, ISC, ideal state criteria,
project specification, articulating done, scaffolding an VSA, interviewing for
an VSA, checking completeness, reconciling an ephemeral feature file back to a
master VSA, or seeding an VSA from an existing project.

Do not use for creating new skills, running the Algorithm itself, generating
non-VSA artifacts, postmortems, decision logs, or engineering journals. Those
are retrospective; the VSA is a commitment-time scaffold.

## Workflow Routing

When ambiguous, default to Scaffold for new VSAs and CheckCompleteness for
audits.

| Intent | Workflow | Load |
| --- | --- | --- |
| scaffold, create, generate, new VSA, extract feature as ephemeral | Scaffold | `Workflows/Scaffold.md` |
...
```text
Running the **WorkflowName** workflow in the **VSA** skill to ACTION...
```
...
- Project VSAs require E3+ structure even when the active task is smaller.
