...
Bootstrap a draft project VSA from an existing repository's README, code structure, recent commits, and any pre-existing PRD-shaped artifacts. Used when a project predates the VSA framework and needs to be brought into the system without inventing fiction.

## When to invoke

- The Algorithm at OBSERVE on a project that has no `<project>/VSA.md` and the task is non-trivial: `Skill("VSA", "seed <project-path>")`
- User directly when onboarding a project: `Skill("VSA", "seed ~/Projects/<repo>")`
...
|-------|----------|-------------|
| project_path | yes | Repository root (where the new `VSA.md` will be written) |
| name | no | Project name; defaults to basename of project_path |
| tier | no | Default E3 (the project VSA minimum). Override to E4/E5 for fully-fleshed bootstrap. |
| dry_run | no | Default false. If true, emit the proposed VSA to stdout instead of writing. |

## Output

A new file at `<project_path>/VSA.md` populated with draft content sourced from the repository. Status report:

```yaml
status: created | dry_run | exists
path: <project_path>/VSA.md
sources_consulted:
  - README.md
  - package.json
  - tsconfig.json
...
curl -s -X POST http://localhost:31337/notify \
  ...
  -d '{"message": "Running the Seed workflow in the VSA skill"}' \
  > /dev/null 2>&1 &
```

### Step 2 — Refuse if VSA already exists

If `<project_path>/VSA.md` exists, abort and emit `status: exists`. Seed never overwrites — the user uses Interview or Scaffold to deepen an existing project VSA.
...
Write `<project_path>/VSA.md` with the drafted sections. Frontmatter:

```yaml
---
project: <name>
task: "Project VSA — <name>"
effort: <tier>
...
Emit `review_required: true` in the report. Seed is a **draft** — the human-author pass is mandatory before the VSA is treated as authoritative. The user's first Algorithm task on the project should run Interview to refine.
...
- **Does not run CheckCompleteness.** Seeded VSAs are explicitly partial; running CheckCompleteness against a seeded VSA at E4 will fail by design.
...
- **Pre-existing VSA detected:** abort with `status: exists`. Refuse to overwrite. Suggest Interview instead.
