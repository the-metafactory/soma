...
function renderToolContract(): string {
  return [
    ...
    "- `memory_recall`: note-aware retrieval over durable memory notes (term-scored whole-note matches + 1-hop links, superseded excluded, verification banners). Prefer this over `memory_search` before durable claims.",
...
function renderHomeSkill(input: ProjectionInput, somaHome: string): string {
  return [
    ...
    "- Use the `soma_context` tool with `action: memory_recall` before making durable claims that may depend on prior knowledge, learning, relationship, work, or imported PAI context — it is note-aware (term-scored notes + 1-hop links, verification banners). `action: memory_search` remains as a legacy line-grep fallback.",
